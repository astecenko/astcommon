...
////////////////////////////////////////////////////////////////////////////////
  ...
  begin
    for i:=1 to length(fn)do field_name[i] := fn[i];
    field_type := ft;
    for i:=1 to 4 do  dummy[i] := #00;
    len := l;
    dec := d;
    for i:=1 to 14 do filler[i] := #00;
  end;
end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  try //обработка исключений (если возникнут)
    ...
    begin
      ...
      last_update.luYear := YearOf(Date) - 2000;//получение года в двухзначном формате
      ...
      data_offset := 32 + 32 * Length(DBF_Fields)+3; //смещение до данных = заголовок + 32*кол-во полей + признак конца описания структуры
      rec_size :=0; //обнуление
      for i:=0 to High(DBF_Fields) do
        rec_size := rec_size + DBF_Fields[i].len;//размер записи = сумма размеров полей + 1 (разделительный байт)
      rec_size := rec_size + 1;
      for i:=1 to 20 do filler[i] := #00;
    end;
...
////////////////////////////////////////////////////////////////////////////////
  try   
    ...
    while (not fStartSI) and (rez = 4) do   // 1 условие для определения начала сл. инф, второе для определения конца файла
    begin
      BlockRead(DBF_File,blok,4,rez); //читаенм по 4 символа да SIA#00
      if (blok[1]='S') and (blok[2]='I') and (blok[3]='A') and (blok[4]= #00)
      then fStartSI := True
			else for j:=1 to 7 do  BlockRead(DBF_File,blok,4,rez); //читаенм еще 7 раза по 4 символа - до начала чледующих 32 байт
    end;
    ...
    begin
      ...
      begin
        if (j - 32*trunc(j/32)) in [1,2,3,4,12,13,17,18] //33 - 32(33/32) =33-32*1=1
        then BlockRead(DBF_File, ch,1)                //пропускаем ненужные позиции
        else
        begin
          BlockWrite(DBF_File,si.si_text[i],1); //записываем служебную информацию
          i:=i+1;
        end;
        j:=j+1;
      end;//while
    ...
    else  PutSI := True; //записи СИ невозможна т.к. не нашлись поля служебной информации
  finally //при любых ситуациях - файл закрыть
    CloseFile(DBF_File);
  end;
end;
  ...
  try
   //чистим переменную
    ...
    begin
      BlockRead(DBF_File,blok,4,rez); //читаенм по 4 символа да SIA#00
      if (blok[1]='S') and (blok[2]='I') and (blok[3]='A') and (blok[4]= #00)
      then fStartSI := True //признак начала служебной информации
      else for j:=1 to 7 do BlockRead(DBF_File,blok,4,rez); //читаем еще 7 раза по 4 символа - до начала чледующих 32 байт
    end;
    ...
    begin
      ...
      si.si_countsi := StrToIntDef(countsi2, 0);  //если конвертирование не пройдет, длину ниж. инф. установим в ноль.
      ...
      begin
        ...
        Case cod of
           ...
           168: strDOS := strDOS+chr(240) //Ё
               Else  strDOS:=strDOS+copy(strWin,i,1);
        end;
      end;
      CodDOS:=strDOS;
end;
...
var i,cod:Integer; strASCII:String;
  begin
      strASCII:='';
      For i:= 1 To Length(strWin) do
      begin
        ...
        Case cod of
           ...
           240: strASCII := strASCII+chr(168) //Ё
               Else  strASCII:=strASCII+copy(strWin,i,1);
        end;
      end;
      CodASCII:=strASCII;
  end;
